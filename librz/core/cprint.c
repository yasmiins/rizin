 ...
 *  ...
 */
static RZ_OWN char *core_print_format(RzCore *core, const char *fmt, const char *value, int mode) {
	...
	if (fmtname) {
		...
		if (format) {
			...
			if (value && comp && !comp->value) {
				comp->value = rz_str_new(value);
			}
		}
	}
	char *realfmt = comp ? rz_str_newf("%s.%s", comp->name, comp->field) : rz_str_new(fmt);
	// If the split into components is finished, use the only format
	int struct_sz = rz_type_format_struct_size(core->analysis->typedb, realfmt, mode, 0);
...
RZ_IPI RZ_OWN char *rz_core_print_format(RzCore *core, const char *fmt, int mode) {
	return core_print_format(core, fmt, NULL, mode);
}

RZ_IPI RZ_OWN char *rz_core_print_format_write(RzCore *core, const char *fmt, const char *value) {
	return core_print_format(core, fmt, value, RZ_PRINT_MUSTSET);
}
